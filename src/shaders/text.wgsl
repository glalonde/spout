...
struct VertexInput {
    ...
    @location(0) pos: vec2<f32>,       // screen-space top-left in pixels
...
fn vs_main(input: VertexInput) -> VertexOutput {
    ...
    // Pixel coordinates → NDC: x: [0, width] → [-1, 1], y: [0, height] → [1, -1]
    let ndc = vec2<f32>(
        pixel_pos.x / screen.size.x * 2.0 - 1.0,
        1.0 - pixel_pos.y / screen.size.y * 2.0,
    );
