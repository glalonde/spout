...
fn main() {
    ...
    let music2 = music::Music::new("a_so_close.ogg").unwrap_or(None);
    if music2.is_some() {
        log::warn!("got music!");
    }

    threads.push(thread::spawn(move || {
        log::info!("Reqwest for music?");
        let music_future = music::get_music();
        wasm_bindgen_futures::spawn_local(async move {
            log::info!("Reqwest started?");
            let _ = music_future.await;
            log::info!("Reqwest done?");
        });
        log::info!("Reqwest done.");
    }));

    // Load music in another thread, and we'll command it asynchronously.
    threads.push(thread::spawn(move || {
        let mut music = music::Music::new("a_so_close.ogg").unwrap_or(None);
        if !music.is_some() {
            log::warn!("No music!");
        }
        loop {
            ...
            if music.is_some() {
                ...
                match received {
                    MusicCommand::Toggle => {
                        log::warn!("Toggling song.");
                        // music.).toggle_song();
                        music.as_mut().unwrap().toggle_song();
                    }
                    MusicCommand::Stop => {
                        log::warn!("Stop not implemented.");
                    }
                }
            }
        }
    ...
    // Wait for all threads, otherwise program exits before threads finish
    // execution. We can't do blocking join on wasm main thread though, but the
    // browser window will continue running.
    #[cfg(not(target_arch = "wasm32"))]
