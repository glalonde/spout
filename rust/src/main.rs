...
impl framework::Example for Example {
    fn init(
        ...
        let vs = spout::include_shader!("particle_system/quad.vert.spv");
        let vs_module =
            device.create_shader_module(&wgpu::read_spirv(std::io::Cursor::new(&vs[..])).unwrap());
        // Renders the data texture onto the canvas.
        let fs = spout::include_shader!("particle_system/quad.frag.spv");
        let fs_module =
            device.create_shader_module(&wgpu::read_spirv(std::io::Cursor::new(&fs[..])).unwrap());

        let texture_extent = wgpu::Extent3d {
            width: system_params.width,
            height: system_params.height,
            depth: 1,
        };
